...
try:
	while True:
		...
		text = (
			...
			f' PCI={pci[-1]} MODE={mode[-1]}'\
			f' ulBandWidth={ulbandwidth[-1]} dlBandWidth={dlbandwidth[-1]}'\
			f' BAND={band[-1]} ULFREQ={ulfrequency[-1]} DLFREQ={dlfrequency[-1]}')
		print(text)
		plt.clf()
		...
		if period_refresh:
			if period_refresh > cycle_time:
				plt.pause(period_refresh-cycle_time)
